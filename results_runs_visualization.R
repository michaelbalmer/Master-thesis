...
##plot performance of FS2 (1 ahead pred) against train days
...
#############################################################################
###Plot performance of Feature set X (all training days) as a function of pred steps

# #####RF
# rf_plot_pred_hor_train <- data.frame(c(1:10))
# for (i in c(1:10)){
#   maes <- run_featureset_two[[i]]$mean_mae
#   rf_plot_pred_hor_train <- cbind(rf_plot_pred_hor_train,maes)
# }
# names(rf_plot_pred_hor_train) <- c('horizon','1d','2d','3d','4d','5d','6d','7d',
#                                    '8d','9d','10d')
# 
# rf_plot_pred_hor_train <- melt(rf_plot_pred_hor_train,id='horizon')
# 
# ggplot(rf_plot_pred_hor_train, aes(horizon,value))+
#   geom_line(aes(colour=variable))+
#   scale_colour_manual(values=c('#a50026','#d73027','#f46d43','#fdae61',
...
#         legend.title=element_text(size=9), 
#         legend.text=element_text(size=8))+
#   ggtitle('MAE on k steps ahead prediction (Feature set 2)')
# 
# ###NN
# nn_plot_pred_hor_train <- data.frame(c(1:10))
# for (i in c(1:10)){
#   maes <- run_featureset_ten_nn[[i]]$mean_mae
#   nn_plot_pred_hor_train <- cbind(nn_plot_pred_hor_train,maes)
# }
# names(nn_plot_pred_hor_train) <- c('horizon','1d','2d','3d','4d','5d','6d','7d',
#                                    '8d','9d','10d')
# 
# nn_plot_pred_hor_train <- melt(nn_plot_pred_hor_train,id='horizon')
# 
# ggplot(nn_plot_pred_hor_train, aes(horizon,value))+
#   geom_line(aes(colour=variable))+
#   scale_colour_manual(values=c('#a50026','#d73027','#f46d43','#fdae61',
#                                '#fee08b','#d9ef8b','#a6d96a','#66bd63',
#                                '#1a9850','#006837'))+
...
###############################################################################
##Feature importance
#names(run_featureset_ten[[1]][6]$imp_incmse[[1]][1:10])
...
plot_importance_mean_occr <- ggplot(data = importances[c(21,1),], aes(x = feature,
                                        y = as.numeric(mean_importance))) +
  theme_bw()+
  geom_bar(stat = "identity", width = .4,fill='#3182bd') +
...
plot_importance_mean_occr

plot_importance_time <- ggplot(data = importances[1,], aes(x = feature,
                                                            y = as.numeric(mean_importance))) +
  theme_bw()+
  geom_bar(stat = "identity", width = .4,fill='#3182bd') +
  labs(x = "", y = "",title='Time') +
  scale_y_continuous(limits=c(0.0,0.07),breaks=seq(0,0.07,0.01),expand = c(0,0))+
  theme(axis.text.x = element_text(size=10,angle = 45,hjust = 1),
        panel.border = element_rect(colour = "black", fill=NA, size=.5),
        plot.title = element_text(vjust=-7,hjust=.5),
...
plot_importance_landuse


###arrange four plots into one
ggarrange(
  plot_importance_mean_occr, plot_importance_centrality,plot_importance_poi,
...
plot_dist_error_5hfs1

street_seg_error_rfnn

try <- street_seg_error_rfnn

try$new <- ifelse(try$BLOCK_ID %in% c(10203,10302), 'this', NA)
try2 <- dataAMJ
try2$new <- month(try2$START_TIME_DT,label = T,abbr = F)
format(try2$new,"%B")
...
plot_dist_improvement

# ###average occupancy rate per district
# ggplot(dataAMJ, aes(x=PM_DISTRICT_NAME, y=OCC_RATE),fill='#3182bd') + 
...
# 
# ###average occupancy rate per district
# ggplot(dataAMJ, aes(x=PM_DISTRICT_NAME, y=PARK_CAPACITY),fill='#3182bd') + 
#   geom_boxplot(width=.5,outlier.size=.8)+
#   labs(x = "", y = "Occupancy Rate")+
#   theme(axis.text.x = element_text(size=10, angle = 45, hjust = 1),
#         legend.position = c(.9, .8),
...
###RF
#plot percentage improvements (1/4)
...
percent_change_horizon_rf <- ggplot(melted_change_table_rf, aes(as.numeric(variable),value,group=id))+
  ...
  theme(plot.title = element_text(size = 12, face = "bold"),
        ...
        legend.text=element_text(size=8),
        legend.box.background = element_rect(colour = "black"),
        legend.position = c(0.81,0.080),legend.direction = 'horizontal',
        panel.grid.minor.x = element_blank(),
        plot.margin = unit(c(0.5,0.5,2,0.5), "cm"),
        legend.key.size = unit(.4,"cm"))+
  guides(colour = guide_legend(title.position = "top"))+
...
###improvement as function of prediction horizon
###ANN
#plot percentage improvements (2/4)
...
percent_change_horizon_nn <- ggplot(melted_change_table_nn, aes(as.numeric(variable),value,group=id))+
  ...
  theme(plot.title = element_text(size = 12, face = "bold"),
        ...
        legend.text=element_text(size=8),
        legend.box.background = element_rect(colour = "black"),
        legend.position = c(0.81,0.066),legend.direction = 'horizontal',
        panel.grid.minor.x = element_blank(),
        plot.margin = unit(c(0.5,0.5,2,0.5), "cm"),
        legend.key.size = unit(.4,"cm"))+
  guides(colour = guide_legend(title.position = "top"))+
...
melted_change_table_rf
melted_change_table_nn

#########

#plot percentage improvements (3/4)### 1 hours ahead prediction
...
percent_change_train_1h_rf <- ggplot(melted_change_table_1hp_rf, aes(as.numeric(variable),
  ...
  theme(plot.title = element_text(size = 12, face = "bold"),
        ...
        legend.text=element_text(size=8),
        legend.box.background = element_rect(colour = "black"),
        legend.position = c(0.81,0.925),legend.direction = 'horizontal',
        panel.grid.minor.x = element_blank(),
        plot.margin = unit(c(0.5,0.5,2,0.5), "cm"),
        legend.key.size = unit(.4,"cm"))+
  guides(colour = guide_legend(title.position = "top"))+
...
#plot percentage improvements (4/4) ### 1 hours ahead prediction
...
percent_change_train_1h_nn <- ggplot(melted_change_table_1hp_nn, aes(as.numeric(variable),
  ...
  theme(plot.title = element_text(size = 12, face = "bold"),
        ...
        legend.text=element_text(size=8),
        legend.box.background = element_rect(colour = "black"),
        legend.position = c(0.81,0.94),legend.direction = 'horizontal',
        panel.grid.minor.x = element_blank(),
        plot.margin = unit(c(0.5,0.5,2,0.5), "cm"),
        legend.key.size = unit(.4,"cm"))+
  guides(colour = guide_legend(title.position = "top"))+
...
melted_change_table_5hp_rf

#######

#plot percentage improvements (5/4) ###5 hours ahead prediction
...
#plot percentage improvements (6/4) ### 5 hours ahead prediction
